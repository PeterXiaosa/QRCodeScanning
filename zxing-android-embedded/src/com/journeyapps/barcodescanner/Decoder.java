...
public class Decoder implements ResultPointCallback {
    ...
    public Result[] decodeMulti(LuminanceSource source){
        return decodeMulti(toBitmap(source));
    }
    ...
    protected Result[] decodeMulti(BinaryBitmap bitmap) {
        possibleResultPoints.clear();
        try {
            if (reader instanceof MultiFormatReader) {
                // Optimization - MultiFormatReader's normal decode() method is slow.
                MultipleBarcodeReader multipleBarcodeReader = new GenericMultipleBarcodeReader(reader);
