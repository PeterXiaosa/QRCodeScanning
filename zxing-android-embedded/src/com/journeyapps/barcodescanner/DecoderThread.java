...
//                decode((SourceData) message.obj);
                decodeMulti((SourceData) message.obj);
    ...
    private void decodeMulti(SourceData sourceData) {
        long start = System.currentTimeMillis();
        Result[] rawResult = null;
        sourceData.setCropRect(cropRect);
        LuminanceSource source = createSource(sourceData);
        ...
        if(source != null) {
            rawResult = decoder.decodeMulti(source);
        }
        ...
        if (rawResult != null) {
            ...
            Log.d(TAG, "Found barcode in " + (end - start) + " ms");
            if (resultHandler != null) {
                Result resultTest = rawResult[0];
                String text = "";
                ResultPoint[] resultPoints = resultTest.getResultPoints();
                BarcodeFormat barcodeFormat = resultTest.getBarcodeFormat();
                for (int i = 0; i < rawResult.length; i ++){
                    if (i != 0 ){
                        text = text +"," + rawResult[i].getText();
                    }else {
                        text = rawResult[i].getText();
                    }
                }
                Result result = new Result(text, resultTest.getRawBytes(), resultPoints, barcodeFormat);
